...
export default function Achievements() {
  ...
  const getIconForAchievement = (iconName: string) => {
    switch (iconName) {
      ...
      case 'calendar-check': return <Calendar className="h-6 w-6" />;
  ...
  const getCategoryName = (category: AchievementCategory): string => {
    switch (category) {
      ...
      case 'consistency': return 'Konsequenz';
      case 'exploration': return 'Erkundung';
      case 'insight': return 'Einsicht';
      case 'mastery': return 'Meisterschaft';
      case 'special': return 'Speziell';
    ...
    <div className="container mx-auto py-6">
      ...
      {newAchievements.length > 0 && (
        <div className="mb-8 p-4 bg-green-100 dark:bg-green-900 rounded-lg">
          <h2 className="text-xl font-semibold mb-3">Neue Erfolge freigeschaltet!</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {newAchievements.map(achievement => (
              <div key={achievement.achievementId} className="flex items-center gap-3 p-3 bg-white dark:bg-gray-800 rounded-md shadow-sm">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600 dark:text-gray-300">{achievement.achievementDescription}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      {Object.entries(groupedAchievements).map(([category, categoryAchievements]) => (
        <div key={category} className="mb-8">
          <h2 className="text-2xl font-bold mb-4">{getCategoryName(category as AchievementCategory)}</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {categoryAchievements.map(achievement => {
              ...
              return (
                <Card key={achievement.id} className={`overflow-hidden ${isCompleted ? 'border-green-500' : ''}`}>
                  <CardHeader className={`relative ${isCompleted ? 'bg-green-50 dark:bg-green-900/30' : ''}`}>
                    {isCompleted && (
                    ...
                    <div className="flex items-center gap-3">
                      <div className={`p-3 rounded-full ${getDifficultyColor(achievement.difficulty)}`}>
                        {getIconForAchievement(achievement.iconName)}
                      </div>
                      <div>
                  ...
                  <CardFooter className="bg-gray-50 dark:bg-gray-800/50 pt-3 pb-3 text-xs text-gray-500">
                    {isCompleted && userAchievement && (
                      <span>Erreicht am {new Date(userAchievement.unlockedAt).toLocaleDateString('de-DE')}</span>
                    )}
                    {(!isCompleted || !userAchievement) && (
      ...
      {Object.keys(groupedAchievements).length === 0 && (
        <div className="text-center p-12">
          ...
          <h3 className="text-xl font-semibold mb-2">Keine Erfolge verfügbar</h3>
