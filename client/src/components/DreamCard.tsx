...
export default function DreamCard({ dream }: DreamCardProps) {
  ...
  const previewText = dream.content.length > 100 
      ...
      <Card className="overflow-hidden hover:-translate-y-1 hover:shadow-lg transition-all cursor-pointer">
        <div className="h-40 bg-gray-200 overflow-hidden relative">
          ...
          <div className="absolute top-3 right-3 bg-white rounded-full p-1 shadow-sm">
            <p className="text-xs font-medium text-gray-600 px-2">{formattedDate}</p>
          </div>
        </div>
        <CardContent className="p-4">
          <h3 className="font-serif font-bold text-lg text-gray-800">{dream.title}</h3>
          <p className="text-gray-600 text-sm mt-1 line-clamp-2">{previewText}</p>
